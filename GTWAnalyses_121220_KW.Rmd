...
Mixed-effects logistic regression examining the effects of language group, age, and temporal location (proximal vs. distal) on children’s likelihood of correctly identifying time words on the first try. 
```{r}
cal.lm = glmer(cor.first ~ language*prox+ageyears*item + (1|subjid), family='binomial', data=cal.d)
summary(cal.lm)
```
...
Mixed-effects logistic regression examining the effects of language group, age, and item ('day before yesterday', 'yesterday', 'tomorrow', 'day after tomorrow') on children’s likelihood of correctly identifying identifying the deictic status of time words. 
```{r}
d.lm = glmer(stat.correct ~ language*prox+ageyears*item + (1|subjid), family='binomial', data=cal.d)
summary(d.lm)
```
...
### plot correct first placements (Did they give correct response first time)
ggplot(data=cal.sum, aes(x=item, y=first.m)) +
  geom_line()+
  geom_point() +
  ylab('prop correct on first try') +
  ylim(0,1) +
  geom_errorbar(aes(ymin=first.m-se.correct, ymax=first.m+se.correct), width=.2, position=position_dodge(.05)) +
  facet_wrap( agegroup ~ language, nrow=3) 
