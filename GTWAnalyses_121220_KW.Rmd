...
summary(d.lm)
```
Calendar Task: does language spoken or age (in years) predict successful placement of time words?
```{r}
...
###Did children place both items in a pair in the correct location? (e.g., tomorrow/after tomorrow, yesterday/before yesterday, proximal terms, distal terms)
```{r}
#create new data frame
cal.allT <- cal.d %>%
  select (subjid, agegroup, language, item, correct)
#transpose item (i.e., transform from long to wide format)
cal.allT <- reshape(cal.e, idvar = c("subjid", "agegroup", "language"), timevar = "item", direction = "wide")
