...
for(sub_i in levels(d.TL2$subjid)){
  for(trial_i in c(2,3,4,6,7,8,10,11,12,13,14,15,16)){
    ...
    RelRankData2 <- rbind(RelRankData, foo)
  }
}

# this is gross but I did it manually in excel 

write.csv(d.TL2,"d.TL2.csv")

# now reading the data back in with new columns 
RelRankData <- read.csv("d.TL2.csv")

RelRankData$ComparedToPrevious <- ifelse(RelRankData$Rank > RelRankData$PrevRank, "R", "L")
RelRankData$CorrComparedToPrevious <- ifelse(RelRankData$CorrectRank > RelRankData$PrevCorrectRank, "R", "L")
RelRankData$AccRelRank <- with(RelRankData, ifelse(ComparedToPrevious == CorrComparedToPrevious, 1, 0))
RelRankData <- subset(RelRankData, ageyears %in% c("3","4","5","6","7","adult"))
...
#xtabs(~BNCorrect + itemtype, RelRankData.agg)
