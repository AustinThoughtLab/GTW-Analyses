...
EnglishStatus.timeline <- ggplot(data=deicticstatus.sum3.english, aes(x=agegroup, y=DS.m, ymin = DS.lower, ymax = DS.upper, group=itemtype, color=itemtype)) +
  ...
  ylim(0,1) +
  scale_color_manual(labels = c("deictic items", "event items"), values = c("orange", "blue"))+
...
#Plot correct status by item type (for each language separately)
# first english-speakers
DS.timeline <- ggplot(data=DS.sum, aes(x=agegroup, y=DS.m, group=language, color=language)) +
...
NBackOrder.timeline <- ggplot(data=BackOrder.sum, aes(x=agegroup, y=BackOrder.m, group=itemtype, color=itemtype, ymin = BackOrder.lower, ymax = BackOrder.upper)) +
  geom_line(aes(color=itemtype))+
  geom_errorbar(width = 0.2) +
  geom_point() +
  ylab('Proportion Correct') +
...
ggsave("BackOrder_PropCorrect.jpeg", NBackOrder.timeline, height = 7, width = 7, dpi = 300)
...
plot.pastfutureGerman2 <- ggplot(timelineRR.sumGer, aes(x=agegroup, y=RelRank.m, ymin = RelRank.lower, ymax = RelRank.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=RelRank.m + RelRank.sd, ymin=RelRank.m - RelRank.sd)) +
  scale_x_discrete(name = "German Speakers' Age (years)") +
  scale_y_continuous(name = "Mean Relative Order Knowledge", breaks= c(.2, .3, .4, .5,.6,.7,.8,.9,1), limits = c(.2, 1)) +
  theme(axis.title = element_text(family = "Times", color = "black", size = 22, hjust = 0.5),
        axis.title.y = element_text(family = "Times", color = "black", size = 22),
        axis.text.y = element_text(family = "Times", color = "black", size = 22), 
        axis.title.x = element_text(family = "Times", color = "black", size = 22),
        axis.text.x = element_text(family = "Times", color = "black", size = 22))
plot.pastfutureGerman2

ggsave("Order_GermanDev_Timeline.png", plot.pastfutureGerman2, dpi = 300, height = 6, width = 6)
...
plot.pastfutureEnglish2 <- ggplot(timelineRR.sumEn, aes(x=agegroup, y=RelRank.m, ymin - RelRank.lower, ymax = RelRank.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=RelRank.m + RelRank.sd, ymin=RelRank.m - RelRank.sd)) +
  scale_x_discrete(name = "English Speakers' Age (years)") +
  ...
  theme(axis.title = element_text(family = "Times", color = "black", size = 22, hjust = 0.5),
        ...
        axis.title.x = element_text(family = "Times", color = "black", size = 22),
        axis.text.x = element_text(family = "Times", color = "black", size = 22))
plot.pastfutureEnglish2

ggsave("Order_EnglishDev_Timeline.png", plot.pastfutureEnglish2, dpi = 300, height = 6, width = 6)
