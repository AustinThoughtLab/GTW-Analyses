...
lib = c("ordinal","gridExtra","MPDiR","tidyr","quickpsy","gmodels", "gplots","tidyverse", "nloptr", "lme4", "ggplot2", "reshape2", "ggpubr", "dplyr", "rstatix", "car", "stats", "coin", "scales", "psych", "ggthemes", "brms", "tidyverse")
...
#summary of performance for deitcit status knowledge
d.test <- d.TL2 %>%
...
#only run the next two lines if running analyses for masters thesis

d.TL2 <- d.TL2 %>%
  filter(agegroup %in% c('4', '5', '6', '7', 'adult'))
...
DeicticStatus_Timeline2 <- d.TL2 %>%
  group_by(itemtype) %>%
  pairwise_wilcox_test(formula = BNCorrect  ~ language.sc, p.adjust.method = "BH") %>%
  add_significance()
DeicticStatus_Timeline2
...
with(subset(d.TL2, agegroup=="3"), t.test(x=BNCorrect, mu=.5)) # better than chance
...
with(subset(d.TL2, agegroup %in% c("3", "4") & language == "english"), t.test(BNCorrect ~ agegroup, var.equal = T)) # 3-year-olds are performing significantly better than 4-year-olds
with(subset(d.TL2, agegroup %in% c("3", "5") & language == "english"), t.test(BNCorrect ~ agegroup, var.equal = T)) # 3-year-olds are performing significantly better than 4-year-olds
with(subset(d.TL2, agegroup %in% c("5", "4") & language == "english"), t.test(BNCorrect ~ agegroup, var.equal = T)) # 3-year-olds are performing significantly better than 4-year-olds
...
```{r item level comparisons for past/future status}
d.TL2_100 <- d.TL2 %>%
  filter(item %in% c("aftertomorrow", "beforeyesterday", "tomorrow", "yesterday"))

d.TL2_100_kids <- d.TL2_100 %>%
  filter(agegroup != "adult" & language == "english") %>%
  mutate(item = as.factor(item))

DS_English <- d.TL2_100_kids %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(BNCorrect ~ item, p.adjust.method = "BH") %>%
  add_significance()
DS_English


Table_english <- d.TL2_100_kids %>%
...
# now ONLY look at timeline #3
#english_rankdata <- RelRankData %>%
  #filter(agegroup != "adult" & language == "english" & agegroup != "3" & item %in% c("beforeyesterday", "yesterday.4","tomorrow.3", "aftertomorrow"))

DS_English2 <- english_rankdata %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(BNCorrect ~ item, p.adjust.method = "BH") %>%
  add_significance()
DS_English2
...
```{r plot proportion correct for past/future status placement (summarize each language separately)}
...
```{r plot proportion correct for past/future status placement (graph each language separately)}
#Plot correct status by item type (for each language separately)
# first english-speakers
EnglishStatus.timeline <- ggplot(data=deicticstatus.sum3.english, aes(x=agegroup, y=DS.m, ymin = DS.lower, ymax = DS.upper, group=itemtype)) +
  geom_line(aes(linetype=itemtype))+
...
GermanStatus.timeline <- ggplot(data=deicticstatus.sum3.german, aes(x=agegroup, y=DS.m, group=itemtype, ymin = DS.lower, ymax = DS.upper)) +
  geom_line(aes(linetype=itemtype))+
  geom_errorbar(width = 0.2) +
  geom_point() +
  scale_linetype_manual(values=c("dotted", "solid"))+
...
ggsave("DS_graphs_4to7.jpeg", arrangeGrob(EnglishStatus.timeline, GermanStatus.timeline), height = 10, width = 7, dpi = 300)
...
#Plot correct status by item type (for each language separately)
# first english-speakers
DS.timeline <- ggplot(data=DS.sum, aes(x=agegroup, y=DS.m, group=language)) +
...
ggsave("DS.timeline.png", DS.timeline, dpi = 300, width = 10, height = 5)
...
timelineDS.sumGer <- d.TL2 %>%
  filter(language == "german") %>%
  group_by(agegroup) %>%
  summarize(n = n(),
            BNCorrect= BNCorrect, 
            deictic1.m = (mean(BNCorrect, na.rm = T)), # who got correct status (first answer)
            deictic1.sd = (sd(BNCorrect, na.rm = T)), # standard deviation
...
plot.pastfutureGerman <- ggplot(data=timelineDS.sumGer, aes(x=agegroup, y=deictic1.m, ymin = deictic1.lower, ymax = deictic1.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=deictic1.m + deictic1.se, ymin=deictic1.m - deictic1.se)) +
...
ggsave("DS_GermanDev_Timeline.png", plot.pastfutureGerman, dpi = 300, height = 6, width = 6)

timelineDS.sumEn <- d.TL2 %>%
  filter(language == "english") %>%
  group_by(agegroup) %>%
  summarize(n = n(),
            BNCorrect= BNCorrect, 
...
plot.pastfutureEnglish <- ggplot(data=timelineDS.sumEn, aes(x=agegroup, y=deictic1.m, ymin = deictic1.lower, ymax = deictic1.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=deictic1.m + deictic1.se, ymin=deictic1.m - deictic1.se)) +
  scale_x_discrete(name = "Age (years)") +
...
ggsave("DS_EnglishDev_Timeline.png", plot.pastfutureEnglish, dpi = 300, height = 6, width = 6)
...
Order_Timeline2 <- d.TL2 %>%
  group_by(agegroup, language) %>%
  pairwise_wilcox_test(formula = RankDif ~ itemtype, p.adjust.method = "BH") %>%
  add_significance()
Order_Timeline2
```
...
```{r item level comparisons for relative order}
english_rankdata <- RelRankData %>%
  filter(agegroup != "adult" & language == "english" & agegroup != "3" & item %in% c("beforeyesterday", "yesterday.4","tomorrow.3", "aftertomorrow"))

Rank_English <- english_rankdata %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(AccRelRank ~ item, p.adjust.method = "BH") %>%
  add_significance()
Rank_English 


Rank_English2 <- english_rankdata %>%
  group_by(item, agegroup) %>%
  summarize(n = n(),
            DS.m = (mean(AccRelRank, na.rm = T))*100, # who got correct status (first answer)
            DS.sd = (sd(AccRelRank, na.rm = T))*100, # standard deviation
            DS.se = DS.sd/sqrt(n), # calculate standard error
            DS.lower = DS.m - DS.se, # calculate lower 95% CI
            DS.upper = DS.m + DS.se) # calculate upper 95% CI
Rank_English2
...
```{r t tests for relative order measure}
#### t-test for order knowledge on this 1-back measure
with(subset(RelRankData, agegroup=="3"), t.test(x=AccRelRank, mu=.5)) # better than chance
...
with(subset(RelRankData, agegroup %in% c("3", "4") & language == "english"), t.test(AccRelRank ~ agegroup, var.equal =T)) # no different
with(subset(RelRankData, agegroup %in% c("4", "5") & language == "english"), t.test(AccRelRank ~ agegroup, var.equal =T)) # sig different performance
with(subset(RelRankData, agegroup %in% c("3", "5") & language == "english"), t.test(AccRelRank ~ agegroup, var.equal =T)) # no diferent
...
EnglishBackOrder.timeline <- ggplot(data=BackOrder.englishsum, aes(x=agegroup, y=BackOrder.m, ymin = BackOrder.lower, ymax = BackOrder.upper, group=itemtype)) +
  geom_line(aes(linetype=itemtype))+
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_errorbar(width = 0.2) +
  geom_point() +
...
GermanBackOrder.timeline <- ggplot(data=BackOrder.germansum , aes(x=agegroup, y=BackOrder.m, group=itemtype, ymin = BackOrder.lower, ymax = BackOrder.upper)) +
  geom_line(aes(linetype=itemtype))+
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_errorbar(width = 0.2) +
  geom_point() +
...
ggsave("BackOrder_graphs_4to7.jpeg", arrangeGrob(EnglishBackOrder.timeline, GermanBackOrder.timeline), height = 10, width = 7, dpi = 300)

```

```{r graph proportion correct for N-back order knowledge collapsed across language groups}
BackOrder.sum <- RelRankData %>%
  group_by(itemtype, agegroup) %>%
  summarize(n = n(),
            BackOrder.m = (mean(AccRelRank, na.rm = T)), # who got correct status (first answer)
            BackOrder.sd = (sd(AccRelRank, na.rm = T)), # standard deviation
...
NBackOrder.timeline <- ggplot(data=BackOrder.sum, aes(x=agegroup, y=BackOrder.m, group=itemtype, ymin = BackOrder.lower, ymax = BackOrder.upper)) +
  geom_line(aes(linetype=itemtype))+
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_errorbar(width = 0.2) +
  geom_point() +
...
ggsave("BackOrder_PropCorrect.jpeg", NBackOrder.timeline, height = 5, width = 7, dpi = 300)

```

```{r plot development of N-back measure of relative order knowledge on timeline task}
timelineRR.sumGer <- RelRankData %>%
  filter(language == "german") %>%
  group_by(agegroup) %>%
  summarize(n = n(),
            AccRelRank= AccRelRank, 
            RelRank.m = (mean(AccRelRank, na.rm = T)), # who got correct status (first answer)
            RelRank.sd = (sd(AccRelRank, na.rm = T)), # standard deviation
            RelRank.se = RelRank.sd/sqrt(n), # calculate standard error
            RelRank.lower = RelRank.m - RelRank.se, # calculate lower 95% CI
            RelRank.upper = RelRank.m + RelRank.se) # calculate upper 95% CI

plot.OrderGerman2 <- ggplot(timelineRR.sumGer, aes(x=agegroup, y=RelRank.m, ymin = RelRank.lower, ymax = RelRank.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=RelRank.m + RelRank.sd, ymin=RelRank.m - RelRank.sd)) +
  scale_x_discrete(name = "") +
  ggtitle("") +
...
ggsave("Order_GermanDev_Timeline.png", plot.OrderGerman2, dpi = 300, height = 6, width = 6)

timelineRR.sumEn <- RelRankData %>%
  filter(language == "english") %>%
  group_by(agegroup) %>%
  summarize(n = n(),
            AccRelRank= AccRelRank, 
...
plot.OrderEnglish2 <- ggplot(timelineRR.sumEn, aes(x=agegroup, y=RelRank.m, ymin - RelRank.lower, ymax = RelRank.upper)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=RelRank.m + RelRank.sd, ymin=RelRank.m - RelRank.sd)) +
  scale_x_discrete(name = "") +
  ggtitle("Relative Order") + 
  scale_y_continuous(name = "", breaks= c(.2, .3, .4, .5,.6,.7,.8,.9,1), limits = c(.2, 1)) +
  theme(plot.title = element_text(family = "Times", color = "black", size = 25, hjust = 0.5),
        axis.title = element_text(family = "Times", color = "black", size = 22, hjust = 0.5),
        axis.title.y = element_text(family = "Times", color = "black", size = 22),
...
ggsave("Order_EnglishDev_Timeline.png", plot.OrderEnglish2, dpi = 300, height = 6, width = 6)
...
MaxLinePlacements <- d.TL2 %>%
  ...
  summarize(n = n(),
            maxDistance = max(abs(distfrommid)))
...
d.TL12$signedScaledDist <- d.TL12$distfrommid/d.TL12$maxDistance # divide each raw distance by the maximum distance an item was placed on the line
