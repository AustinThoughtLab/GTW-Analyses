...
d.TL2$BNStatus <- with(d.TL2,ifelse(item %in% c("breakfast", "lastbday", "lastweek", "thismorning", "yesterday.3","yesterday.4", "lastyear", "beforeyesterday"), 1, 0))
...
t.lm1 <- glmer(BNCorrect ~ language.sc *itemtype.sc*scale(ageyears) + (1|subjid), family = 'binomial', data=d.TL2, control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) # language, age, and itemtype are significant predictors of BNCorrect
summary(t.lm1) 
...
d.TL2EN <- d.TL2 %>%
  filter(language == "english" & agegroup %in% c("4", "5", "6", "7", "adult"))
...
DeicticStatus_EnglishTimeline <- d.TL2EN %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(formula = BNCorrect ~ itemtype, p.adjust.method = "BH") %>%
  add_significance()
DeicticStatus_EnglishTimeline

d.TL2GERM <- d.TL2 %>%
  filter(language == "german" & agegroup %in% c("4", "5", "6", "adult"))

DeicticStatus_GermanTimeline <- d.TL2GERM %>% 
  group_by(agegroup) %>%
  pairwise_wilcox_test(formula = BNCorrect ~ itemtype, p.adjust.method = "BH") %>%
  add_significance()
DeicticStatus_GermanTimeline # I can't get this to run, maybe too many NAs?

# across both languages, within each agegroup what was performance like?
deicticstatus.sum <- d.TL2 %>%
  group_by(agegroup) %>%
  summarize(n = n(),
...
# now within each language
deicticstatus.sum1 <- d.TL2 %>%
...
```{r plot proportion correct for past/future status placement}
...
EnglishStatus.timeline <- ggplot(data=deicticstatus.sum3.english, aes(x=agegroup, y=DS.m, group=itemtype, color=itemtype)) +
...
GermanStatus.timeline <- ggplot(data=deicticstatus.sum3.german, aes(x=agegroup, y=DS.m, group=itemtype, color=itemtype)) +
  geom_line(aes(color=itemtype))+
  geom_point() +
  ylab('Proportion Correct') +
...
d.TL2$TrialNum <- with(d.TL2,ifelse(order==1,
                                      match(item,c("breakfast","nextbday","dinner","lastbday","lastweek","tomorrow.2",
...
plot.data.agg <- aggregate(avgRankDif ~ agegroup, plot.data, mean)
plot.data.agg$varRank <- aggregate(avgRankDif ~ agegroup, plot.data, sd)$avgRankDif

plot.RankDif <- ggplot(data=plot.data.agg, aes(x=agegroup, y=avgRankDif)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.5, linetype="dashed",color="red",size=1.3) +
  geom_pointrange(size=1.5, position=position_dodge(width=0.3), aes(ymax=BNCorrect + varRank, ymin=BNCorrect - varRank)) +
  scale_x_discrete(name = "Age (years)") +
  scale_y_continuous(name = "avgRankDif ", breaks= c(.5,.6,.7,.8,.9,1), limits = c(.45, 1)) +
  theme(axis.title = element_text(size=20))
plot.RankDif 
```
...
ggsave("meantimelineplacements_english.jpeg", width = 20, height = 15)
```
...
ggsave("meantimelineplacements_german.jpeg", width = 20, height = 15)
```
