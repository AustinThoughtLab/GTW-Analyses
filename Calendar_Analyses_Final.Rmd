...
CorrectStatPlacements <- ggplot(data=cal.sumProp, aes(x=agegroup, y = deictic1.m, group = item)) +
  ...
  geom_point() +
  ylab('Proportion Correct (Past/Future Status)') +
  xlab('Age') +
  ylim(0,1) +
...
verbal.sum1 <- d.all %>%
  filter(itemtype == "verbal" & item == verbal_items & agegroup %in% c('4', '5', '6', '7')) %>% #filter only 5 verbal questions
  group_by(language, agegroup, item) %>%
...
# does age or language predict Nichiren's performance on the 4 verbal questions (Excluding days of the week)?
...
# english only 
verbal.sum1english <- d.all %>%
  filter(itemtype == "verbal" & language == "english" & agegroup %in% c('4', '5', '6', '7') & item == verbal_items) %>% #filter only 5 verbal questions
  group_by(language, agegroup, item) %>%
  summarize(correct.m.v = (mean(correct, na.rm=T)), # % who answered correctly
            sd.correct.v = (sd(correct, na.rm=T)), #calculate standard deviation
            n = n(),
