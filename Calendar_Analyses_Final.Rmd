...
#prox.sc (1 - proximal, 0 = distal)

deictic.sum2.0 <- calv.d %>%
  filter(itemtype == "deictic" & agegroup %in% c('4', '5', '6', '7')) %>%
  group_by(language, agegroup, prox.sc) %>%
  summarize(correct.m.d = mean(cor.first, na.rm=T), # % who answered correctly
            sd.correct.d = sd(cor.first, na.rm=T),
            n = n(),
            se.correct.d = sd.correct.d/sqrt(n),
...
calhist.e <- cal.d %>% #filter english speakers and create new data frame
  filter(language == "english" & agegroup %in% c('3', '4','5','6', '7')) %>%
  mutate(item = factor(item, levels = c("beforeyesterday", "yesterday", "tomorrow", "aftertomorrow")))

calhist.g <- cal.d %>% #filter German speakers and create new data frame
  filter(language == "german" & agegroup %in% c('3', '4','5','6', '7')) %>%
  mutate(item = factor(item, levels = c("beforeyesterday", "yesterday", "tomorrow", "aftertomorrow")))

calhist.e$agebin[calhist.e$agegroup == '3'] <- "Age 3"
...
calhist.g$agebin[calhist.g$agegroup == '7'] <- "Age 7"
...
# Graph Correct First Placement (but show proportion of kids who placed sticker in each box)
# with 3- and 7-year-olds added back in
cal.sum5 <- cal.d %>%
  mutate(item = factor(item, levels = c("beforeyesterday", "aftertomorrow","yesterday","tomorrow"))) %>%
  group_by(language, agegroup, item, response1) %>%
  summarize(n = n()) %>%
...
```{r predicting past/future placement w/ages 3-7}
...
#if running analyses for master's thesis
cal.d <- cal.d %>%
...
# does language spoken, proximity, or age (in years) predict successful placement of time words in the past or the future? 
# model with sum coded predictor variables
d.lm2 <- glmer(stat.correct.1 ~ language.sc*prox.sc*scale(ageyears) + (1|subjid), family = 'binomial', data=cal.d, control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) 
# once I increased the # of possible iterations the optimizer is allowed to have (i.e. to 100,000) convergence issues disappear 
summary(d.lm2) 

Anova(d.lm2, contrasts=list(topic=contr.sum, sys=contr.sum), type=3) # only age is a significant predictor

DeicticStatus_prox <- cal.d %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(formula = stat.correct.1 ~ prox.sc, p.adjust.method = "BH") %>%
  add_significance()
DeicticStatus_prox # sig difference at age 6 (p = .05)
```
...
ggsave("CalCorrectPlacementProp.jpeg",CorrectStatPlacements, dpi = 300, width = 10, height = 7)
...
```{r prox analyses with past/future status knowledge}
...
summary(remote1)
Anova(remote1, contrasts=list(topic=contr.sum, sys=contr.sum), Type=3)
#Anova(remote1) # main effect of item (yesterday, tomorrow), main effect of age, but no interactions (why are these main effects only showing up in the Anova?)

All_remotelang <- cal.d %>% # conduct Wilcox_test 
  group_by(agegroup) %>% 
  pairwise_wilcox_test(formula = cor.remote ~ language.sc, p.adjust.method = "BH") %>%
  add_significance()
All_remotelang


#does language spoken, proximity, or age (in years) predict successful placement of time words the correct # of squares from today? [interaction term included in the model]
remote2 <- glm(cor.remote ~ language.sc*prox.sc*scale(ageyears), family = binomial(link = "logit"), data=cal.d) 
summary(remote2)# main effect of proximity and age, but no effect of language
Anova(remote2, contrasts=list(topic=contr.sum, sys=contr.sum), Type=3)

All_remote <- cal.d %>% # conduct Wilcox_test 
  group_by(agegroup) %>% 
  pairwise_wilcox_test(formula = cor.remote ~ prox, p.adjust.method = "BH") %>%
  add_significance()
All_remote 
...
cal.sum5Figure6$proxFac <- case_when(cal.sum5Figure6$prox == 1 ~ "Proximal",
                                     cal.sum5Figure6$prox == 0 ~ "Distal")


CorrectRemoteness<- ggplot(data=cal.sum5Figure6, aes(x=agegroup, y = remoteness.m, group = proxFac)) +
  geom_line(aes(linetype=proxFac)) +
...
ggsave("CalCorrectRemotenessProp.jpeg",CorrectRemoteness, dpi = 300, width = 10, height = 7)
```
...
# remove all interaction terms [when we do this, all the main effects are present]
...
preciseLang <- cal.d %>%
  group_by(agegroup) %>%
  pairwise_wilcox_test(formula = cor.first  ~ language.sc, p.adjust.method = "BH") %>%
  add_significance()
preciseLang
...
PreciseKnowledge_Prop<- ggplot(data=cal.sum5Figure7, aes(x=agegroup, y = preciseknow.m, group = language)) +
...
ggsave("CalPreciseKnowledge_PropProp.jpeg",PreciseKnowledge_Prop, dpi = 300, width = 10, height = 7)
```

```{r graph prop correct for distal terms only }
cal.sum5Figure7.5 <- cal.d %>%
  filter(prox != 1) %>%
  group_by(agegroup, language) %>%
  summarize(n = n(),
            preciseknow.m = (mean(cor.first, na.rm = T)), # who got correct status (first answer)
...
PreciseKnowledge_Prop2<- ggplot(data=cal.sum5Figure7.5, aes(x=agegroup, y = preciseknow.m, group = language)) +
  geom_line(aes(linetype = language)) +
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_point() +
  ylab('Proportion Correct (Distal Terms)') +
  xlab('Age') +
  ylim(0,1) +
  theme_minimal(base_size = 14) +
  theme(plot.title=element_text(family = "Times", color = "black", size = 14, hjust = 0.5), 
...
PreciseKnowledge_Prop2

ggsave("CalPreciseKnowledge_PropProp2.jpeg",PreciseKnowledge_Prop2, dpi = 300, width = 10, height = 7)
```

```{r graph prop correct for proximal terms only }
cal.sum5Figure7.6 <- cal.d %>%
  filter(prox != 0) %>%
  group_by(agegroup, language) %>%
  summarize(n = n(),
            preciseknow.m = (mean(cor.first, na.rm = T)), # who got correct status (first answer)
...
PreciseKnowledge_Prop3<- ggplot(data=cal.sum5Figure7.6, aes(x=agegroup, y = preciseknow.m, group = language)) +
  geom_line(aes(linetype = language)) +
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_point() +
  ylab('Proportion Correct (Proximal Terms)') +
  xlab('Age') +
  ylim(0,1) +
  theme_minimal(base_size = 14) +
  theme(plot.title=element_text(family = "Times", color = "black", size = 14, hjust = 0.5), 
...
PreciseKnowledge_Prop3

ggsave("CalPreciseKnowledge_PropProp3.jpeg",PreciseKnowledge_Prop3, dpi = 300, width = 10, height = 7)
```

```{r graph prop correct for each language group separately}
...
cal.sum5Figure7G$proxFac <- case_when(cal.sum5Figure7G$prox == 1 ~ "Proximal",
                                     cal.sum5Figure7G$prox == 0 ~ "Distal")

PreciseKnowledge_PropG<- ggplot(data=cal.sum5Figure7G, aes(x=agegroup, y = preciseknow.m, group = proxFac)) +
  geom_line(aes(linetype=proxFac)) +
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_point() +
  ylab('Proportion Correct') +
...
cal.sum5Figure7E$proxFac <- case_when(cal.sum5Figure7E$prox == 1 ~ "Proximal",
                                     cal.sum5Figure7E$prox == 0 ~ "Distal")

PreciseKnowledge_PropE<- ggplot(data=cal.sum5Figure7E, aes(x=agegroup, y = preciseknow.m, group = proxFac)) +
  geom_line(aes(linetype=proxFac)) +
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_point() +
  ylab('Proportion Correct') +
...
p.lm3 <- glmer(cor.first ~ language.sc*item_stat.sc*scale(ageyears) + (1|subjid), family = 'binomial', data=cal.d, control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000))) 
summary(p.lm3) 
...
# plot
plot.deicticEnglish <- ggplot(data=cal.sumProp2, aes(x=agegroup, y=deictic1.m)) + 
  ...
  ggtitle("Deictic Status") +
  scale_y_continuous(name = "Proportion Correct ", breaks= c(.2, .3, .4, .5,.6,.7,.8,.9,1), limits = c(.2, 1)) +
  theme(plot.title = element_text(family = "Times", color = "black", size = 25, hjust = 0.5),
        axis.title = element_text(family = "Times", color = "black", size = 22, hjust = 0.5),
        axis.title.y = element_text(family = "Times", color = "black", size = 22),
        axis.text.y = element_text(family = "Times", color = "black", size = 22), 
        axis.title.x = element_text(family = "Times", color = "black", size = 22),
        axis.text.x = element_text(family = "Times", color = "black", size = 22))
plot.deicticEnglish

ggsave("plot.caldeicticEnglish.png", plot.deicticEnglish, dpi = 300, height = 6, width = 6)
...
plot.remotenessEnglish <- ggplot(data=cal.sum5.2, aes(x=agegroup, y=remoteness.m)) + 
  # geom_hline(yintercept=1, linetype="solid",color="black",size=1.3) +
  geom_hline(yintercept=.33, linetype="dashed",color="red",size=1.3) +
  ...
  scale_x_discrete(name = "Age (years)") +
  ggtitle("Temporal Remoteness") +
  scale_y_continuous(name = "", breaks= c(.2, .3, .4, .5,.6,.7,.8,.9,1), limits = c(.2, 1)) +
    theme(plot.title = element_text(family = "Times", color = "black", size = 25, hjust = 0.5),
        axis.title = element_text(family = "Times", color = "black", size = 22, hjust = 0.5),
        axis.title.y = element_text(family = "Times", color = "black", size = 22),
        axis.text.y = element_text(family = "Times", color = "black", size = 22), 
        axis.title.x = element_text(family = "Times", color = "black", size = 22),
        axis.text.x = element_text(family = "Times", color = "black", size = 22))
plot.remotenessEnglish

ggsave("plot.CalremotenessEnglish.png", plot.remotenessEnglish, dpi = 300, height = 6, width = 6)
...
ggsave("plot.CalpreciseknowEnglish.png",plot.preciseknowEnglish, dpi = 300, height = 6, width = 6)
```
...
ggsave("plot.CalDSGerman.png", plot.DSGerman, dpi = 300, height = 6, width = 6)
...
ggsave("plot.CalremotenessGerman .png", plot.remotenessGerman , dpi = 300, height = 6, width = 6)
...
ggsave("plot.CalpreciseknowGerman.png",plot.preciseknowGerman, dpi = 300, height = 6, width = 6)
```

```{r verbal questions: days of the week}

verbal <- d.all %>%
  filter(itemtype == "verbal" & item == "daysofweek" & agegroup %in% c('4', '5', '6', '7')) %>%
  #group_by(agegroup) %>%
  summarize(n =  n(),
            Gcorrect.m = mean(correct, na.rm = T),
            Ecorrect.m = mean(DaysOfWeek_Correct, na.rm = T)) 
verbal

verbal_data <- calv.d %>%
  filter(itemtype == "verbal" & item == "daysofweek" & agegroup %in% c('4', '5', '6', '7')) %>%
  mutate(correct_DOW = case_when(language == "german" ~ correct,
                                 language == "english" ~ DaysOfWeek_Correct))

# does age or language predict children's performance on the days of the week question?

DOW.lm1 <- glmer(correct_DOW ~ language.sc*scale(ageyears) + (1|subjid), family = binomial(link='logit'), data=verbal_data, control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))
summary(DOW.lm1)
Anova(DOW.lm1, Type =3) # only age in years significantly predicts correct response to verbal Q's
...
verbal.long <- reshape(verbal, idvar = "agegroup",
                       varying=c("Gcorrect.m", "Ecorrect.m"),
                       v.name = c("value"),
                       times=c("Gcorrect.m", "Ecorrect.m"),
                       new.row.names = 1:10,
                       direction = "long")

cbbPalette <- c("#D55E00", "#CC79A7") # colorblind friendly palette


# graph long data 
DaysOFWeek <- ggplot(verbal.long, aes(x = agegroup, y = value, fill = time)) + 
  geom_bar(
           stat= "identity",
           position = "dodge") + 
  labs(x = "Age", y = "Proportion Correct", title = ('Days of the Week')) +
  scale_fill_manual(name="", 
                        labels = c("English Speakers", 
                                   "German Speakers"),
                        values = c("Ecorrect.m"="#CC79A7",
                                   "Gcorrect.m"="#D55E00")) +
  #scale_fill_discrete(name = " ", labels = c("English Speakers", "German Speakers")) +
  ylim(0, 1) + 
  theme_classic()

ggsave("DaysoftheWeek.png", DaysOFWeek, dpi = 300, width = 7, height= 5)
...
```{r overall performance on verbal questions - quick summary}
verbal_items <- c("aftertoday", "beforetoday", "yesterday", "tomorrow", "today")
#recalculate correct column for verbal questions only 
verbal.d <- d.all %>%
    filter(itemtype == "verbal" & agegroup %in% c('4', '5', '6', '7') & item != "daysofweek") %>% #filter only 5 verbal questions
    mutate(correct = case_when(correctR == response ~ 1,
                               correctR != response ~ 0))
...
verbal.sum1 <- verbal.d %>%
  group_by(language, agegroup, item) %>%
  summarize(n = n(),
            correct.m.v = mean(correct, na.rm=T), # % who answered correctly
...
# does age or language predict chilren's performance on the 4 verbal questions (Excluding days of the week)?

verbal.lm1 <- glmer(correct ~ language.sc*scale(ageyears) + (1|subjid), family = binomial(link='logit'), data=verbal_data2, control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)))
summary(verbal.lm1)
Anova(verbal.lm1, Type =3) # age in years, language, and their interaction significantly predicts correct response to verbal Q's

# plot proportion correct (collapsed across languages)
VerbalQuestions_Prop<- ggplot(data=verbal.sum1, aes(item, correct.m.v, fill = language)) +
  ...
  xlab('') +
  scale_x_discrete(labels = c("Which day comes after today?", "What day is it tomorrow?", "What day is it today?","What day was it yesterday?","Which day comes before today?")) +
  #ylim(0,1) +
  theme_minimal(base_size = 14) +
  theme(plot.title=element_text(family = "Times", color = "black", size = 14, hjust = 0.5), 
        axis.title.y = element_text(family = "Times", color = "black", size = 14),
        ...
        axis.text.x = element_text(family = "Times", color = "black", size = 14)) +
          ...
          theme(panel.border = element_rect(colour = "black", fill = NA, size = 1),
                panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                axis.line = element_line(colour = "black"))
VerbalQuestions_Prop

ggsave("VerbalQuestions_Prop.jpeg",VerbalQuestions_Prop, dpi = 300, width = 10, height =5)
```

```{r overall performance on verbal questions - english only}
# english only 
verbal.sum1english <- verbal_data2  %>%
  filter(language == "english") %>%
  group_by(language, agegroup, item) %>%
  summarize(n = n(),
            correct.m.v = (mean(correct, na.rm=T)), # % who answered correctly
...
VQ_PropEnglish<- ggplot(data=verbal.sum1english, aes(item, correct.m.v)) +
  geom_col() +
  facet_wrap(~agegroup) +
  coord_flip() +
  ylab('Proportion Correct (English Speakers)') +
  scale_y_continuous(breaks =  seq(0, 1, by  = .5)) +
  xlab('Item') +
  scale_x_discrete(labels = c("Which day comes after today?", "What day is it tomorrow?", "What day is it today?","What day was it yesterday?","Which day comes before today?")) +
  theme_minimal(base_size = 14)
...
ggsave("VQ_PropEnglish.jpeg",VQ_PropEnglish, dpi = 300, width = 10, height =5)

# german only 
verbal.sum1german <- d.all %>%
  filter(itemtype == "verbal" & language == "german" &agegroup %in% c('4', '5', '6', '7') & item == verbal_items) %>% #filter only 5 verbal questions
  group_by(language, agegroup, item) %>%
...
VQ_PropGerman<- ggplot(data=verbal.sum1german, aes(item, correct.m.v)) +
  geom_col() +
  facet_wrap(~agegroup) +
  coord_flip() +
  ylab('Proportion Correct (German Speakers)') +
  scale_y_continuous(breaks =  seq(0, 1, by  = .5)) +
  xlab('Item') +
  scale_x_discrete(labels = c("Which day comes after today?", "What day is it tomorrow?", "What day is it today?","What day was it yesterday?","Which day comes before today?")) +
  theme_minimal(base_size = 14) #+
...
ggsave("VQ_PropGerman.jpeg",VQ_PropGerman, dpi = 300, width = 10, height =5)
```

```{r verbal questions part II}
...
```{r verbal questions - each age?}
verbal.5 <- d.all %>%
  filter(itemtype == "verbal" & agegroup == "5" & language == "english", item == "tomorrow")

verbal.6 <- d.all %>%
  filter(itemtype == "verbal" & agegroup == "6" & language == "english")
```
...
verbal.deictic <- glm(correct ~ itemtype.sc*language.sc*scale(ageyears), family = binomial(link='logit'), data=calv.d) #run logistic regression
summary(verbal.deictic)
Anova(verbal.deictic)

# followup analyses to explore main effects found above
...
itemTest_D <- calv.d %>%
  group_by(itemtype) %>%
  pairwise_wilcox_test(formula = correct  ~ language.sc, p.adjust.method = "BH") %>%
  add_significance()
itemTest_D
```

```{r graph prop correct on verbal vs. deictic questions}

calv.d.combined <- calv.d %>%
  filter(agegroup != "adult") %>%
  mutate(correct_CalCombined = case_when(itemtype == "verbal" ~ correct,
                             itemtype == "deictic" ~ stat.correct.1))
cal.sumCombined <- calv.d.combined %>%
  group_by(itemtype, language) %>%
  summarize(n = n(),
            correct.m = (mean(correct_CalCombined, na.rm = T)), # correct responses
            correct.sd = (sd(correct_CalCombined, na.rm = T)), # standard deviation
            correct.se = correct.sd/sqrt(n), # calculate standard error
            correct.lower = correct.m - correct.se, # calculate lower 95% CI
            correct.upper = correct.m + correct.se) # calculate upper 95% CI
cal.sumCombined

PropCorrectGraph<- ggplot(data=cal.sumCombined, aes(x=agegroup, y = correct.m , group = language)) +
  geom_line(aes(linetype = language)) +
  facet_wrap(~itemtype) +
  scale_linetype_manual(values=c("dotted", "solid"))+
  geom_point() +
...
ggsave("ItemType_PropLang.jpeg",PropCorrectGraph, dpi = 300, width = 10, height = 7)
```
...
calhist.e$agebin[calhist.e$agegroup == '5'] <- "Age 5"
calhist.e$agebin[calhist.e$agegroup == '6'] <- "Age 6"
calhist.e$agebin[calhist.e$agegroup == '7'] <- "Age 7"
calhist.g$agebin[calhist.g$agegroup == '3'] <- "Age 3"
calhist.g$agebin[calhist.g$agegroup == '4'] <- "Age 4"
calhist.g$agebin[calhist.g$agegroup == '5'] <- "Age 5"
calhist.g$agebin[calhist.g$agegroup == '6'] <- "Age 6"
calhist.g$agebin[calhist.g$agegroup == '7'] <- "Age 7"
...
English_SquarePlacements <- ggplot(calhist.e, aes(x = factor(response1), fill = ColorBar)) +
  geom_bar() +
  #scale_fill_grey(end = 0.3, na.value = "grey50") +
  ...
  ylim(0, 35) +
  ggtitle("English Speakers (n = 141)") +
...
ggsave(Figure2, file= "Figure2.jpeg", width = 15, height = 8, dpi = 300)
```
...
```{r Plot Proportion Correct - sticker in correcvt box on first try}
