...
summary(cal.lm.e)
Anova(cal.lm.e, Type=3) #suggests main effects, no interaction
...
### plot correct status by ietm (for each language separately)
...
```
```{r}
# now let's compare performance between time words (within each language separately)
...
#compute summary statitistics by item: median and interquartile range

Englishcal_sum %>%
  group_by(item) %>%
  get_summary_stats(first.m, type = "median_iqr")
