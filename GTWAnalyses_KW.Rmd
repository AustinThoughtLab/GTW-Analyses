...
d.all <- d.all %>%
  ...
  mutate(response = as.numeric(response),
         ...
         resp.stat.1 = case_when(task=='timeline' & distfrommid > 0 ~ 1,
                               ...
                               TRUE ~ 0), # codes deictic status of the *first* sticker placement
         ...
         stat.correct.1 = ifelse(item.stat == resp.stat.1, 1, 0)) #codes whether deictic status of first sticker placement was correct
```
...
```{r}
cal.d <- d.all %>%
#filter only calendar items that are marked as deictic (verbal items not involved in the calendar task)
...
cal.d2 <- d.all %>%
#filter only calendar items that are marked as deictic (verbal items not involved in the calendar task)
  filter(task=="calendar" & itemtype=="deictic" & agegroup %in% c('4','5','6') & ageyears<7.0) %>% 
  select(-linelength,-distfrommid) %>%
  mutate(cor.first = ifelse(response1==correctR, 1, 0), # did they get it right on the first trial
         prox = ifelse(item %in% c('yesterday','tomorrow'),1,0), # yesterday and tomorrow coded as proximal terms
         prox = as.factor(prox))

#exclude 3- and 7-year-olds from cal.sum
cal.sum <- cal.d2 %>%
  ...
  summarize(first.m = mean(cor.first, na.rm=T),     # % who put item in right box on first try
            ...
            deictic1.m = mean(stat.correct.1), #for got correct status (first answer)
...
cal.d %>%
  mutate(item = factor(item, levels = ("beforeyesterday", "yesterday", "tomorrow", "aftertomorrow")))
```
...
```{r}
#calhist.e <- calhist.e %>%
  #mutate(CorrectSquare = case_when(item == "tomorrow"~ "5",
                          #item == "yesterday"~ "3",
                          #item == "aftertomorrow"~ "6",
                          #item == "beforeyesterday"~ "2"))
#I was going to try to use CorrectSquare to color an individual bar within each faceted graph. Haven't figured out if this will work yet. 

English_SquarePlacements <- ggplot(calhist.e, aes(x = factor(response1))) +
  geom_bar() +
  ...
  theme(
      strip.text.x = element_text(
        size = 14, color = "black", family = "Times"),
...
German_SquarePlacements <- ggplot(calhist.g, aes(x = factor(response1))) +
  geom_bar() +
  #scale_fill_manual(values = c()) +
  theme_minimal(base_size = 14) +
  facet_grid(item~agebin) +
...
ggsave(Figure2, file= "Figure2.jpeg", width = 10, height = 8, dpi = 300)
```
...
Does age or item predict successful placement of time words in the past vs. the future within each langauge group separately? [no interaction term]
```{r}
...
cal.e2 <- cal.d %>%
  filter(language == "english" & agegroup %in% c('4','5','6') & ageyears<7.0) %>%
  droplevels() %>%
  select(agegroup,language, item, cor.first) %>%
  mutate(item = factor(item, levels = c("beforeyesterday", "yesterday", "tomorrow", "aftertomorrow")))

cal.e2$age <- as.character(cal.e2$agegroup)
...
cal.g2$age <- as.character(cal.g2$agegroup)

#Wilcox_test for item level comparisons
English.test <- cal.e2 %>%
  group_by(age) %>%
  pairwise.wilcox.test(cal.e2$cor.first, cal.e2$item, p.adjust.method = "BH")
English.test
...
English.test.effsize <- cal.e %>% wilcox.effsize(cdor.first~item)
English.test.effsize
...
German.test
#calculate effect size
German.test.effsize <- cal.g %>% wilcox.effsize(cor.first ~item)
German.test.effsize

?wilcox.test
```

Calendar Task: does language spoken, temporal location (proximal vs. distal),  or age (in years) predict successful placement of time words in the correct squares on the first try? [interaction term included in the model]
```{r}
...
Calendar Task: does language spoken, temporal location (proximal vs. distal),  or age (in years) predict successful placement of time words in the correct squares on the first try? [interaction term removed from the model]
```{r}
...
Mixed-effects logistic regression examining the effects of age, and temporal distance on successful placement of time words in the correct squares on the first try [in English only]
```{r}
cal.lm.e1 = glmer(cor.first ~ prox*scale(ageyears) + (1 + item|subjid), family='binomial', data=cal.e)
summary(cal.lm.e1)
Anova(cal.lm.e1, Type=3) #suggests main effects, no interaction

cal.lm.e2 = glmer(cor.first ~ prox + scale(ageyears) + (1|subjid), family='binomial', data=cal.e)
summary(cal.lm.e2)
Anova(cal.lm.e2, Type=3)
```

Mixed-effects logistic regression examining the effects of age, and temporal distance on successful placement of time words in the correct squares on the first try [in German only]
```{r}
cal.lm.g1 = glmer(cor.first ~ prox*scale(ageyears) + (1|subjid), family='binomial', data=cal.g)
summary(cal.lm.g1)
Anova(cal.lm.g1, Type=3)

cal.lm.g2 = glmer(cor.first ~ prox + scale(ageyears) + (1|subjid), family='binomial', data=cal.g)
summary(cal.lm.g2)
...
EnglishPlacements <- ggplot(data = Englishcal_sum, aes(x=agebin, y = first.m, group = item, color = item)) +
...
GermanPlacements <- ggplot(data=Germancal_sum, aes(x=agebin, y = first.m, group = item, color = item)) +
  geom_line(aes(color=item)) +
  geom_point() +
  ylab('Proportion Correct') +
...
ggsave(CorrectFirstFigure, file= "BoxPlacementByItem2.jpeg",  width = 10, height = 10, dpi = 300)
```


```{r}
### plot correct status
correctStatus <- ggplot(data=cal.sum, aes(x=agegroup, y=deictic1.m, group=language, color=language)) +
  geom_line(aes(color=language))+
  geom_point() +
  ylab('Proportion Correct') +
  ylim(0,1) +
