...
library(stats) # for regular regression glm and lm
library(coin)
library(scales)  # for scale_y_continuous(label = percent)
library(psych) # for calculating ICC reliability check on timeline task
library(ggthemes) # for scale_fill_few('medium')

# to correctly run Type III tests, the way R codes factors has to be changed from its default
...
d.lm3 <- glmer(stat.correct.1 ~ prox.ec*language.ec*scale(ageyears) + (0 + prox.ec|subjid), family = 'binomial', data=cal.d)
...
d.lm3.0 <- glm(stat.correct.1 ~ prox.ec*language.ec*scale(ageyears), family = 'binomial', data=cal.d)
summary(d.lm3.0) 
cal.d$language = as.factor(cal.d$language)
