...
Calendar Task: does langauge, item or age (in years) predict successful placement of time words in the correct squares on the first try?
```{r}
cal.lm.both <- glmer(cor.first ~ language + item + ageyears + (1|subjid), family = 'binomial', data=cal.d) # repeat of cal.lm2?
summary(cal.lm.both)
Anova(cal.lm.both, Type=3)
```

Mixed-effects logistic regression examining the effects of age, and item on successful placement of time words in the correct squares on the first try [for each language group separately]
```{r}

#English participants only
cal.lm.e1 = glmer(cor.first ~ item*scale(ageyears) + (1 + item|subjid), family='binomial', data=cal.e)
summary(cal.lm.e1)
Anova(cal.lm.e1, Type=3) #suggests main effects, no interaction

cal.lm.e2 = glmer(cor.first ~ item + scale(ageyears) + (1|subjid), family='binomial', data=cal.e)
summary(cal.lm.e2)
Anova(cal.lm.e2, Type=3)

#German participants only
cal.lm.g1 = glmer(cor.first ~ item*scale(ageyears) + (1|subjid), family='binomial', data=cal.g)
summary(cal.lm.g1)
Anova(cal.lm.g1, Type=3)

cal.lm.g2 = glmer(cor.first ~ item + scale(ageyears) + (1|subjid), family='binomial', data=cal.g)
summary(cal.lm.g2)
...
```{r}
#These models are redundant (see above)
